...
pub trait IRBuilder {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult;
}
...
impl IRBuilder for parser::ParsingResult {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        match self {
            &Ok((ref ast, _)) => ast.codegen(context, jitter_context),
...
impl IRBuilder for Vec<parser::ASTNode> {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        ...
        for node in self.iter() {
            result = Ok(try!(node.codegen(context, jitter_context)));
        }
...
impl IRBuilder for parser::ASTNode {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        match self {
            &parser::ExternNode(ref prototype) => prototype.codegen(context, jitter_context),
            &parser::FunctionNode(ref function) => function.codegen(context, jitter_context)
        }
    }
}
...
impl IRBuilder for parser::Prototype {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        ...
        let function = match jitter_context.get_module().get_function_by_name(&self.name) {
            ...
            None => {
                ...
                FunctionRef::new(&mut jitter_context.get_module(), &self.name, &fty)
            }
        };
...
impl IRBuilder for parser::Function {
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        ...
        let (function, _) = try!(self.prototype.codegen(context, jitter_context));
        ...
        // redefine it
        let body = match self.body.codegen(context, jitter_context) {
            Ok((value, _)) => value,
            Err(message) => {
        ...
        jitter_context.get_pass_manager().run(&mut function);
    ...
    fn codegen(&self, context: &mut Context, jitter_context: &mut jitter::Context) -> IRBuildingResult {
        match self {
            ...
            &parser::UnaryExpr(ref operator, ref operand) => {
                let (operand, _) = try!(operand.codegen(context, jitter_context));

                let name = "unary".to_string() + operator;
                let function = match jitter_context.get_module().get_function_by_name(name.as_str()) {
            ...
            &parser::BinaryExpr(ref name, ref lhs, ref rhs) => {
                ...
                if name.as_str() == "=" {
                    ...
                    let (value, _) = try!(rhs.codegen(context, jitter_context));
                ...
                let (lhs_value, _) = try!(lhs.codegen(context, jitter_context));
                let (rhs_value, _) = try!(rhs.codegen(context, jitter_context));
                ...
                match name.as_str() {
                    ...
                    op => {
                        ...
                        let function = match jitter_context.get_module().get_function_by_name(&name) {
                            Some(function) => function,
            ...
            &parser::CallExpr(ref name, ref args) => {
                ...
                for arg in args.iter() {
                    let (arg_value, _) = try!(arg.codegen(context, jitter_context));
            ...
            &parser::ConditionalExpr{ref cond_expr, ref then_expr, ref else_expr} => {
                let (cond_value, _) = try!(cond_expr.codegen(context, jitter_context));
                ...
                let (then_value, _) = try!(then_expr.codegen(context, jitter_context));
                ...
                let (else_value, _) = try!(else_expr.codegen(context, jitter_context));
                context.builder.build_br(&merge_block);
            ...
            &parser::LoopExpr{ref var_name, ref start_expr, ref end_expr, ref step_expr, ref body_expr} => {
                let (start_value, _) = try!(start_expr.codegen(context, jitter_context));
                ...
                let (end_value, _) = try!(end_expr.codegen(context, jitter_context));
                let zero = RealConstRef::get(&context.ty, 0.0);
                ...
                try!(body_expr.codegen(context, jitter_context));

                let (step_value, _) = try!(step_expr.codegen(context, jitter_context));
            ...
            &parser::VarExpr{ref vars, ref body_expr} => {
                ...
                for var in vars.iter() {
                    ...
                    let (init_value, _) = try!(init_expr.codegen(context, jitter_context));
                ...
                let (body_value, _) = try!(body_expr.codegen(context, jitter_context));
